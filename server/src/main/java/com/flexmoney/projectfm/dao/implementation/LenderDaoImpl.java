...
public class LenderDaoImpl implements LenderDao {
    ...
    public List<Lender> fetchLenders(Integer mobile, BigDecimal amount) {
        return jdbcTemplate.query(
                "SELECT * FROM lenders l " +
                        "JOIN pa_user_lender p ON p.lender_id = l.lender_id " +
    ...
    public String fetchTenures(List<Lender> lenderList) {
        String sql = "SELECT t.* FROM tenure t " +
                "JOIN lender l ON t.lender_id = l.lender_id " +
        ...
        List<Tenure> tenures = jdbcTemplate.query(sql, new MapSqlParameterSource[]{parameters}, new BeanPropertyRowMapper<>(Tenure.class));
